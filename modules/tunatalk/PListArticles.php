...
$query = "call PListArticles({$idUser}, '{$articleList}');";
...
while($row = $res->fetch_object()) {
	...
	$centerBody .= <<< EOD
		<div class='showArticleList'>
			<div class='left'>
			<a href='?p=showmessage&amp;articleId={$articleId}' class='listNav' >{$title}</a>
			<br /><br />
			<p class='articleInfo'>Skriven av: {$author}, senast uppdaterad: {$articleDate}</p>
			</div>
			<div class='right'>
				<a href='?p=showmessage&amp;articleId={$articleId}' alt='' class='listNav'>Läs</a>
			</div>
			<div class='clear'></div>
		</div>
EOD;
