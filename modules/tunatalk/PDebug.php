	...
	<script type="text/javascript">
  		...
  		var autosave = {
						...
						showResponse: function(data, status) {
							$.jGrowl(data.statement + status + ' <br />articleId: ' + data.articleId + '<br />parentId: ' + data.parentId + '<br />Tid: ' + data.timestamp);
...
$rightBody = <<< EOD
	{$articleNav}
	<br />
		<form id="attach" name="attach" action="?m=files&amp;p=attachfile&amp;idart={$topicId}" method="post" style='display:inline;'>
			<button type="submit" value='submit' id='submitfile'>Bifoga fil</button>
		</form>
		<form id="delete" name="delete" action="?m=files&amp;p=attachfile&amp;idart={$topicId}&amp;delete=true" method="post" style='display:inline;'>
